   ...
   do j=1,nlat
      do i=1,nlon
         if (fvmask(i,j) > 0. .and. fvsst(i,j) .ge. -90.0) then
            ...
            if(hrrrice(i,j) >= xice_threshold .and. fvice(i,j) < xice_threshold ) then  ! turn old seaice into water
                ivgtyp(i,j)=17
                lu_index(i,j)=17
                hrrrmask(i,j)=0.
                xland_rr(i,j)=2.
            ...
            elseif(hrrrice(i,j) < xice_threshold .and. fvice(i,j) >= xice_threshold .and. hrrrsfcT(i,j) < 280.) then  ! turn old water into seaice
                ivgtyp(i,j)=15
                lu_index(i,j)=15
                hrrrmask(i,j)=1.
                xland_rr(i,j)=1.
